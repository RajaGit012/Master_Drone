...
class BayesianOptimization(object):
    ...
    def initialize(self, points_dict):
        ...
        for i in range(len(points_dict['target'])):
            ...
            for key in self.space.keys:
                x.append(points_dict[key][i])
            self.space.add_observation(x, points_dict['target'][i])
            if self.verbose:
                self.plog.print_step(x, points_dict['target'][i])
    ...
    def maximize(self,
        ...
        x_max = acq_local_max(ac=self.util.utility,
                              ...
                              random_state=self.random_state, d=d) if local_acq_opt else \
